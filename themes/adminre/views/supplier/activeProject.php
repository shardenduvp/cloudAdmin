   ...
   <header class="navbar navbar-fixed-top bg-light np header-left">
    ...
    <section id="content" class="table-layout animated fadeIn" style="">
        ...
        <aside class="aside-left-panel col-md-3 va-t pn">
			<div class="animated-delay <?php if(false){ ?> slimscroll-long <?php } ?>layout-left" data-animate='["100","fadeIn"]'>
								...
								<div id="pdetail" class="tab-pane">
									<div class="col-md-12 pl30 pb30 vp-bg pr20 pt25">
										...
										<div class="col-md-8 np mt10">
											<h3 class="font_newlight nm blue-new mb10 mt5"><?php echo $project->clientProjects->clientProfiles->users->first_name; ?> <?php echo $project->clientProjects->clientProfiles->users->last_name; ?></h3>
											<span class="fs12 mr10 display_block">Founder and CEO</span>
										</div>
									</div>
									<div class="col-md-12 np mt20">
										<div class="col-sm-12 np">
											<h4 class="font_newregular pl30 fs12 display_inline  blue-new mb5">Client Info</h4>
										</div>
										<div class="col-md-12 bt-dark pl30 pt20 pb20">
											<a class="fs18 font_newregular team-text-blue" href=""><?php echo $project->clientProjects->clientProfiles->users->company_name; ?></a>
											<div class="col-sm-12 np mt10">
												...
												<span class="col-sm-12 np mt5 mb5"><span class="icon-user mr5 icon-grey-color" aria-hidden="true"></span> <?php echo $project->clientProjects->clientProfiles->team_size; ?> Employee </span>
											</div>
										</div>
										<div class="col-sm-12 np">
													...
													<?php
														...
														foreach($project->clientProjects->clientProjectDocuments as $ref)
														{
															...
															<div class="ref-files" id="ref-<?php echo $ref->id; ?>" >
																<a href="http://<?php echo $ref->path; ?>" target="_blank" class="orange-new"><span class="icon-link fs12" aria-hidden="true"></span> <?php echo $ref->name; ?></a>
																	<div class="pull-right ref-file-show">
																	<a <?php echo $displayNone; ?> onClick="editReferenceFile(<?php echo $ref->id; ?>)" title="Edit"><span aria-hidden="true" class="icon-pencil orange-new mr10"></span></a>
																	<a id="delRef-<?php echo $ref->id; ?>" onClick="deleteReferenceFiles(<?php echo $ref->id; ?>)" title="Delete"><span aria-hidden="true" class="icon-trash orange-new mr5"></span></a>
																	<a id="undoRef-<?php echo $ref->id; ?>" onClick="undoReferenceFiles(<?php echo $ref->id; ?>)" style="" title="Undo">Undo</a>
																</div>
															</div>
															<?php
															}
...
<div class="modal fade" id="createMilestones" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <?php //$this->renderPartial('ajaxMilestones',array("milestone"=>$milestone,"pitch"=>$pitch));  ?>
...
<input type="hidden" id="introID" value="<?php if(isset($_REQUEST['pid'])) echo $_REQUEST['pid']; ?>" />
<input type="hidden" id="clientProjectId" value="<?php echo $project->clientProjects->id; ?>">
 ...
 $(document).ready(function(){
	//$(".tab-slide").slideDown(1500);
			...
			$('.supplier-popover').each(function() {
              ...
              $('#buttonMilestones').on("click",function(e){
        		...
        		if($('#milestonesForm').parsley().validate() == true){
        			...
        			$.ajax({
        				type: 'POST',
        				url:"<?php echo CController::createUrl('/supplier/activeProject');?>",
        				data:data,
        				success :function(data){
        					...
        					if(response.hasOwnProperty('error')){
                                ...
                                $.ajax({
                    				type: 'POST',
                    				url:"<?php echo CController::createUrl('/supplier/renderMilestones');?>",
                    				data:"pId=" + pitchId,
                    				success :function(data){
		   ...
		   $(".milestoneStatus").click(function(){
			    var id = this.id;
				id = id.split("-");				
				$.ajax({
					type: 'POST',
					url:"<?php echo CController::createUrl('/supplier/milestonerequestfund');?>",
					data:"id=" + id[1],
					success :function(data){
						var response = JSON.parse(data);
						alert(response.success);
						$("#mile-" + id[1]).html("New Text");
					}
				});
		...
		function addReferenceFiles(docLink,docName,refType)
		{
			...
			$.ajax({
				type: 'POST',
				url:"<?php echo CController::createUrl('/supplier/AddReferenceFiles');?>",
		...
		function editReferenceFile(id)
		{
			$.ajax({
				type: 'GET',
				url:"<?php echo CController::createUrl('/supplier/ajaxfetchreferenceFiles');?>",
				data:"id=" + id,
				success :function(data){
					var response = JSON.parse(data);
